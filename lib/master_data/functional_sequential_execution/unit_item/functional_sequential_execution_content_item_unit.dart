...
import 'package:frame_creator_v2/master_data/functional_sequential_execution/unit_item/status/functional_sequential_execution_content_item_unit_active_status.dart';

class FunctionalSequentialExecutionContentItemUnit with ExecutionCore {
  ///
  /// TODO:
  ///
  FunctionalSequentialExecutionContentItemUnit({required FlowController? flowController}) {
    setFlowController(value: flowController, isPriorityOverride: true);
    setActiveStatus(value: FunctionalSequentialExecutionContentItemUnitActiveStatus.asDefault(), isPriorityOverride: true);
  }
  ...
  /// -----
  FunctionalSequentialExecutionContentItemUnitActiveStatus? _activeStatus;
  FunctionalSequentialExecutionContentItemUnitActiveStatus? get getActiveStatus => _activeStatus;
  void setActiveStatus({required FunctionalSequentialExecutionContentItemUnitActiveStatus? value, bool? isPriorityOverride}) {
    if (isPriorityOverride == true) {
      _activeStatus = value;
    } else {
      _activeStatus ??= value;
  ...
  /// TODO:
  /// -----
  void addGap({required int? seconds, required FlowController? flowController}) {
    getFunctionalSequentialExecutionStepItemStateListAsStack?.add(
      FunctionalSequentialExecutionStepItemState(
        stateModel: FunctionalSequentialExecutionStepItem.asGapFlow(flowController: flowController)
          ..setPerformId(value: '[PERFORM_ID__]', isPriorityOverride: true)
          ..onSetupRoot()
          ..onInitRoot()
          ..onStart = () {
            if (kDebugMode) {
              print('__________[GAP]');
            }
          ...
          ..getGapTime?.setSeconds(value: seconds ?? 1, isPriorityOverride: true),
      ),
    );
  }
