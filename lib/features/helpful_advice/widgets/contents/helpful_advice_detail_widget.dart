...
class HelpfulAdviceDetailWidget extends StatefulWidget {
  const HelpfulAdviceDetailWidget({super.key, required this.systemStateManagement, required this.sizeDx, required this.sizeDy});
  ...
  @override
  State<HelpfulAdviceDetailWidget> createState() => _HelpfulAdviceDetailWidgetState();
}

class _HelpfulAdviceDetailWidgetState extends State<HelpfulAdviceDetailWidget> with SingleTickerProviderStateMixin {
  Timer? _timer;
  ...
  /// TODO:
  /// -----
  HelpfulAdviceDataModel? _helpfulAdvice;
  HelpfulAdviceDataModel? get getHelpfulAdvice => _helpfulAdvice;
  void setHelpfulAdvice({required HelpfulAdviceDataModel? value, bool? isPriorityOverride}) {
    if (isPriorityOverride == true) {
      _helpfulAdvice = value;
    } else {
      _helpfulAdvice ??= value;
    }
  ...
  final int totalSecondsConst = 60;

  @override
  void initState() {
    super.initState();

    setHelpfulAdvice(value: widget.systemStateManagement?.getHelpfulAdviceFeature?.getCurrentHelpfulAdvice, isPriorityOverride: true);
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
        if (totalSeconds > 0) {
          ...
          totalSeconds = totalSecondsConst;
          widget.systemStateManagement?.getHelpfulAdviceFeature?.onDeactivateWindow();
        }
      });
