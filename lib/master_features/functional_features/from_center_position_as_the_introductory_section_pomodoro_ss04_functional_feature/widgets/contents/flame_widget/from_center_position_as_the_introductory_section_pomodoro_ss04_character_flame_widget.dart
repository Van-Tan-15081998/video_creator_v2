...
class FromCenterPositionAsTheIntroductorySectionPomodoroSS04CharacterFlameWidget extends FlameGame with PositionedCharacterMixin {
  FromCenterPositionAsTheIntroductorySectionPomodoroSS04CharacterFlameWidget({required this.sizeDx, required this.sizeDy, required this.coreFeature});
  ...
  Future<void> onLoad() async {
    ...
    final imageSS02 = await images.load('sprite_sheet/11A/monster11A.png');
    ...
    spriteComponentSS01 = SpriteAnimationComponent()
      ...
      ..position = Vector2(sizeDx - (sizeDxCharacterSS01 / 2 + 25.0), sizeDy - (sizeDyCharacterSS01 / 2 + 50));
    ...
    spriteComponentSS02 = SpriteAnimationComponent()
      ...
      ..position = Vector2(sizeDx - 250.0, sizeDy / 2);
    ...
    pomodoroTitle = TextComponent(
      text: 'PLAY CHESS',
      position: Vector2(sizeDx + sizeDx / 2, sizeDy / 2), // vị trí trên màn hình
      textRenderer: TextPaint(
        // style: GoogleFonts.sriracha(color: Color(0xFFFFE7BA).withValues(alpha: 0.8), fontWeight: FontWeight.bold, fontSize: 50.0, letterSpacing: 5.0),
  ...
  void update(double dt) {
    ...
    if (pomodoroTitle.position.x > (sizeDx * 0.2)) {
